...
def _schema_event_types() -> list[str]:
    """The event alphabet, DERIVED from the published closed set (D599).

    It used to be a hand-copied list of five, and a second hand-copied list of
    eleven lived in scenario() — so seven of the schema's twenty-one types were
    never generated by either, and the one real divergence between the two
    implementations sat in one of the seven. A fuzzer's alphabet copied by hand
    from a registry drifts from it exactly like any other copy (D338).

    No fallback on purpose: a fuzzer that quietly narrows its alphabet when it
    cannot read the registry is the failure this fixes, wearing a helpful face.
    """
    path = os.path.join(ROOT, "spec", "state.schema.json")
    with open(path) as f:
        enum = json.load(f)["$defs"]["ledgerEvent"]["properties"]["event"][
            "properties"]["type"]["enum"]
    if len(enum) < 15:
        raise SystemExit(
            f"differential: the event enum in {path} holds {len(enum)} types — "
            "too few to be the real registry; refusing to fuzz a narrow alphabet")
    return sorted(enum)


EVENT_TYPES = _schema_event_types()
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        tokens = ["yes", "on", "no", "off", "y", "n", "Yes", "YES",
                  ...
                  "0o17", "010", "0b101", "null", "~",
                  # D608: the EMPTY scalar. Absent from this list, and the class it
                  # belongs to (how each side RESOLVES an ambiguous form) is exactly
                  # what this block exists to fuzz — the reference read it as "" and
                  # the runtime as null, giving one file two identities.
                  ""]
        ...
        raw_tmpl = (
            ...
            "  body:\n    note: __TOK__\n"  # block form: an EMPTY token stays empty here
        )
        for tok in tokens:
            diff.check_raw_hash(
