...
|---|---|---|---|
| Four-valued verifier, typed scalars, no-coercion | **proven** | measured | Dual impl (Go + Python) through one 588-case conformance suite — 291 of the 588 run through both, the rest Go-only; Strong-Kleene + injective canonicalization adversarially audited (D178–D179). The strongest claim in the system. |
...
| In-place remediation (converge restores a posture, not a replacement) | **built** | config-intent | D1200–D1213 turned a family of security/retention controls from attributes the tool only OBSERVES-and-blocks into ones `converge` can RESTORE without replacing the resource — because for a stateful resource a replacement destroys the data, rotates identity, and needs consent a patch does not, so "the tool can only fix this by replacing your database" means, in practice, "the tool cannot fix this". Five control families across AWS + Azure: `network.publicExposure`→private (Azure Blob D1200, Postgres D1205, Redis D1206, AI Search D1207; AWS Redshift Serverless D1208); `encryption.inTransit`/TLS (Azure Redis D1209, Postgres D1210; AWS OpenSearch D1213); `retention.window` (AWS Kinesis D1211, Azure Event Hubs D1212); a WORM-aware `retention.minimum` floor (blob D1201/D1202, unlocked PUT / locked one-way `:extend`); and `durability.class` (blob D1203, sync SKU PATCH vs async migration). Each is `classify`=mutable + a wired update that re-checks ownership by tags BEFORE any write, is four-valued on an ambiguous outcome, is enrolled in the foreign-update refusal register, and — where the change is async and security-CLOSING — POLLS to the applied state so a plaintext resource is never reported HTTPS-only while it still speaks plaintext (D953). **The honest gap: this is `built`, not `proven`.** Every path is unit/httptest-measured per driver; NONE has been run against a live account, and the mechanisms are NOT equally proven — a server-parameter write, a property PATCH, a policy-config POST+poll, a stream retention Increase/Decrease, a child PUT preserving partitions and a 72-hour SKU migration are different acts behind one verdict. Where a control is GENUINELY create-fixed it stays an honest replacement (VPC-placement exposure; RDS/ElastiCache/Memorystore TLS; OpenSearch VPC placement), and one retention-shaped knob — Cosmos/Firestore point-in-time recovery, a one-way backup-mode migration — is deferred pending a direction design. |
...
| MCP server (gated tools, two-step apply) | **built** | config-intent | Protocol + gating pinned; hardened (token consume, symlink confinement, and D319: the confirmation token binds the whole decision — plan AND target — after an audit found it could be spent redirecting the same plan at another ledger/provider). The protocol has since been driven END TO END against the shipped binary over stdio (D1254): initialize, tools/list with and without the env gate, then the two-step — token issued, spent, refused on reuse, refused when the plan is swapped (both hashes named), refused when the target is swapped. The evidence class deliberately does NOT move: `measured` in this document means run against a real cloud, and this touches none. Still not exercised by a real external MCP client at scale. |
