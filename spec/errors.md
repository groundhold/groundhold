...
| `ledger-version-ahead` | 2 | run a NEWER build against this ledger, or upgrade this one: it holds an event type this build does not know, and the event-type registry is additive-only, so a later groundhold wrote it. Deliberately NOT `ledger-corrupted`: the remediations are opposites. This refusal says the events cannot be INTERPRETED here, and it states separately whether the hash chain verifies — where it does, the file is undamaged and must not be repaired, quarantined or restored on account of it (D1154) |
