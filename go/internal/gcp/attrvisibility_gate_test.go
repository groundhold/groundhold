...
// D471: the D470 property on gcp. Scope is the FILE the observe lives in, not the
...
// honesty turned out to be real and near-universal: every GCP service that realises an
...
func TestTheVisibilityDetectorFindsAViolationWhenThereIsOneGcp(t *testing.T) {
	build := `
		switch path {
