...
var gcpConsumedOperands = map[string][]string{
	...
	// D763: kmsKeyName and masterAuthorizedCidrs were READ by this driver
	// (gke.go:245 and :263) and listed nowhere, so the guard refused them — while
	// the driver's own refusals told the author to supply exactly those two. Two
	// refusals pointing at each other, and the compiler's sentence ("not an operand
	// the gcp/gke driver reads") was false.
