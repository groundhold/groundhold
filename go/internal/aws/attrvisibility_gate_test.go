...
// D470: an attribute the driver REALISES must be one its observe can either read back or
// say why it cannot.
//
...
// honesty turned out to be real and near-universal: every AWS service that realises an
...
// D784. The sweep above can only fail when a driver ALREADY realises an attribute its
// observe never mentions — and today none does, which is the point of having fixed them.
// So disabling its assertion entirely changes nothing, and no mutant aimed at the sweep
// can bite. That is not the D328 vacuity (a gate with no subject); it is a gate whose
// subject set is legitimately EMPTY.
//
// The machinery still has to work on the day a violation appears, and nothing proved it
// did. This drives the detector against a SYNTHETIC pair: a builder body that realises two
// attributes and an observe body that mentions one. A mutant on realisedAttrs or on the
// comparison fails here even while the real sweep stays silent.
func TestTheVisibilityDetectorFindsAViolationWhenThereIsOne(t *testing.T) {
	build := `
		switch path {
	...
	if !realised["encryption.atRest"] || !realised["retention.minimum"] {
		t.Fatalf("the detector missed a realised attribute: %v — a sweep that cannot SEE "+
			"a violation cannot report one (D784)", realised)
	}
	var silent []string
